...
export default function Dashboard() {
  ...
  const [activeView, setActiveView] = useState<'cards' | 'charts' | 'list'>('cards');

  const load = async () => {
    setLoading(true);
    try {
      const { items } = await apiFetch("/files");
      setItems(items);
    } catch (e: any) {
      alert(e.message);
    } finally {
    ...
    <div className="p-4 sm:p-6 max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 sm:mb-6 space-y-3 sm:space-y-0">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-sm font-medium transition-colors ${activeView === 'cards' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
            onClick={() => setActiveView('cards')}
          >
            Category Cards
          </button>
          <button
            className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-sm font-medium transition-colors ${activeView === 'list' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
            onClick={() => setActiveView('list')}
          >
            Documents
          </button>
          <button
            className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-sm font-medium transition-colors ${activeView === 'charts' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
        ...
        <>
          {activeView === 'cards' ? (
            <>
                ...
                {categoryData.map((category, index) => (
                  ...
                  >
                    <div className="flex justify-between items-center mb-3 sm:mb-4">
                      ...
                      <span className="text-xl sm:text-2xl font-bold">{category.count}</span>
                    </div>
                    <div className="text-xs sm:text-sm text-gray-600 mb-3 sm:mb-4">
                      {category.count} document{category.count !== 1 ? 's' : ''} in this category
                    </div>
                    <a 
                      href={`#${category.name}`} 
              ...
              {activeView === 'list' ? (
                <>
                  <h2 className="text-lg sm:text-xl font-semibold mb-3 sm:mb-4">All Documents</h2>
                  <div className="space-y-3 sm:space-y-4">
                    {items.map((it) => (
                      <div key={it._id} className="bg-white p-3 sm:p-5 rounded-xl shadow-md border">
                        <div className="flex items-center justify-between mb-2 sm:mb-3">
                          <div className="flex items-center gap-1 sm:gap-2">
                            ...
                            <span className="font-medium text-sm sm:text-base">{it.filename}</span>
                            <span className="text-xs sm:text-sm text-gray-500">{it.sourceType}</span>
                          </div>
                        </div>
                        ...
                        <div className="mb-2 sm:mb-3">
                          ...
                          <div className="flex flex-wrap gap-1 sm:gap-2">
                            {Array.isArray(it.categories) ? it.categories.map((category, idx) => (
                              <span 
                                key={idx} 
                          ...
                          >
                            {updatingId === it._id ? (
                              <>
                                ...
                                <span className="hidden sm:inline">Re-summarizing...</span>
                              ...
                              <>
                                ...
                                <span className="hidden sm:inline">Re-run Summary</span>
            ...
            <>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-8">
                ...
                <div className="bg-white p-3 sm:p-6 rounded-xl shadow-md">
                  <h3 className="text-base sm:text-lg font-semibold mb-2 sm:mb-4">Category Distribution</h3>
                ...
                {/* Bar Chart */}
                <div className="bg-white p-3 sm:p-6 rounded-xl shadow-md">
                  <h3 className="text-base sm:text-lg font-semibold mb-2 sm:mb-4">Documents per Category</h3>
                  <div className="h-60 sm:h-80">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart
                        data={barChartData}
